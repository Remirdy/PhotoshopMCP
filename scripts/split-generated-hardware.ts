...
const INPUT = path.join(ROOT, "exports", "knit-sort-layered", "generated_hardware_sheet.png");
const OUT = path.join(ROOT, "exports", "knit-sort-layered", "generated_hardware");
const names = ["clamp_long", "clamp_medium", "clamp_short", "cap_vertical", "hinge_pin", "gate_bridge"];
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    for (let y = 0; y < img.height; y++) {
      for (let x = 0; x < cropW; x++) {
        ...
        if (dist(raw.data[idx], raw.data[idx + 1], raw.data[idx + 2]) > 84) {
    ...
    for (let p = 0; p < data.data.length; p += 4) {
      ...
      if (d < 58) {
        ...
        data.data[p + 3] = Math.min(data.data[p + 3], Math.round(((d - 58) / 130) * 255));
    ...
    fs.writeFileSync(path.join(OUT, `${names[i]}.png`), out.toBuffer("image/png"));
  }

  console.log(`Wrote ${names.length} hardware assets to ${OUT}`);
}
